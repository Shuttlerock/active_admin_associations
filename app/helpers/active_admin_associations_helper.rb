module ActiveAdminAssociationsHelper
  ...
  def admin_form_for(record)
    active_admin_form_for [:admin, record] do |f|
      f.semantic_errors
      f.inputs(*active_admin_config.form_columns) if active_admin_config.form_columns.present?
  ...
  def page_entries_info(collection, options = {})
    ...
    entries_name ||= entry_name.pluralize

    if collection.total_pages < 2
      case collection.size
      when 0 then I18n.t('active_admin.pagination.empty', model: entries_name)
      when 1 then I18n.t('active_admin.pagination.one', model: entry_name)
      ...
      I18n.t('active_admin.pagination.multiple', model: entries_name, from: (offset - collection.size + 1),
to: offset > total ? total : offset, total: total)
    end
  end
end
